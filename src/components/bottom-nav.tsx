...
export function BottomNav() {
  return (
    <nav className="app-bottom-nav safe-bottom border-t border-t-taupe/70 bg-parchment/95 px-4 pb-4 pt-3 backdrop-blur-sm">
      <ul className="app-bottom-nav-list grid grid-cols-4 gap-2 rounded-[1.6rem] bg-cream/90 p-1.5 shadow-paper">
        {items.map((item) => {
          ...
          return (
            <li key={item.to}>
              ...
              >
                ...
                <span className="app-bottom-nav-label block w-full text-center leading-tight">{item.label}</span>
              </NavLink>
            </li>
          )
        })}
